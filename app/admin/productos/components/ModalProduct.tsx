...
export default function ModalProducto({ isOpen, onClose, onSuccess, productoEditar }: Props) {
  ...
  // 1. CARGA DE DATOS: Categorías y Tasa desde Base de Datos (datos_empresa)
  useEffect(() => {
    async function initData() {
      ...
      try {
        ...
        // OBTENER TASA DESDE SUPABASE (Tabla datos_empresa)
        ...
        if (empresa?.tasa_dolar) {
          ...
          // Fallback a API si no hay nada en DB
  ...
  // 2. CÁLCULO AUTOMÁTICO: Precio Bs basado en la tasa cargada
  useEffect(() => {
    const usd = parseFloat(formData.precio)
    if (!isNaN(usd) && tasaDolar > 0) {
      const calculo = (usd * tasaDolar).toFixed(2)
  ...
  // 3. SINCRONIZACIÓN DE FORMULARIO
  useEffect(() => {
    if (!isOpen) return
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (productoEditar?.id) {
        const { error } = await supabase.from('productos').update(productData).eq('id', productoEditar.id)
      ...
      <div className="bg-white w-full max-w-2xl rounded-[2.5rem] shadow-2xl overflow-hidden border border-orange-50 animate-in zoom-in duration-300">
        ...
        <div className={`p-8 border-b border-orange-50 flex justify-between items-center ${productoEditar ? 'bg-blue-50/30' : 'bg-orange-50/30'}`}>
          <div className="flex items-center space-x-4">
            ...
            <div>
              <h3 className="text-2xl font-black text-[#3D1A14] leading-tight">
        ...
        <form onSubmit={handleSubmit} className="p-8 space-y-6 max-h-[75vh] overflow-y-auto custom-scrollbar">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="md:col-span-2">
              ...
              <input required type="text" className={inputStyle} placeholder="Ej: Cuaderno"
            ...
            <div>
              ...
              <input required type="number" step="0.01" className={inputStyle} placeholder="0.00"
            ...
            <div className="relative">
              ...
              <div className="relative">
                <input readOnly type="text" 
                  className={`${inputStyle} bg-blue-50/30 border-blue-100 text-blue-700 cursor-not-allowed font-black`} 
                  value={formData.precio_bs ? `${formData.precio_bs}` : ''} 
                  placeholder="---"
                />
            ...
            <div>
              <label className="flex text-sm font-black text-[#3D1A14] mb-2 ml-1 items-center uppercase tracking-tighter">
                <Hash size={14} className="mr-1" /> Stock Inicial
              </label>
              <input required type="number" className={inputStyle} placeholder="0"
              ...
              <div className="relative h-[65px]">
                {!formData.imagen_url ? (
                  <button type="button" onClick={() => fileInputRef.current?.click()} disabled={uploading}
                    className="w-full h-full border-2 border-dashed border-orange-200 rounded-2xl flex items-center justify-center bg-orange-50/50 hover:bg-orange-50 transition-all font-bold text-gray-500">
            ...
            <div className="md:col-span-2">
              ...
              <textarea rows={2} className={`${inputStyle} resize-none`} placeholder="Describe las características principales..."
                value={formData.descripcion} onChange={e => setFormData({...formData, descripcion: e.target.value})} />
            </div>
          </div>

          <div className="flex space-x-4 pt-4">
            <button type="button" onClick={onClose} className="flex-1 py-4 bg-gray-100 text-[#3D1A14] font-black rounded-2xl hover:bg-gray-200 transition-all active:scale-95 uppercase tracking-tighter text-sm">
              Descartar
            </button>
            <button type="submit" 
              disabled={loading || uploading || categoriasDB.length === 0} 
